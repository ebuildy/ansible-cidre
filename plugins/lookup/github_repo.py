...
- name: Display test-cidre project ID
  debug: msg={{ lookup('community.cidre.github_repo', 'test-cidre') }}
"""
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        arg_endpoint = self.get_option('github_url')
        arg_access_token = self.get_option('github_access_token')

        ret = []

        for term in terms:
            full_url = "%s/repos/%s" % (arg_endpoint, term)

            display.vvv(full_url)
